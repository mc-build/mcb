...
class Parser {
	...
	function gobbleBinaryExpression():Expression {
		...
		var binaryOperatorInfo:{_operator:String, precedence:Int};
		var left:Expression;
		...
		if (right == null) {
			throw new Error('expected expression after binary _operator: "$binaryOperator"', inputString, index);
		}
		...
		while ((binaryOperator = gobbleBinaryOperator()) != null) {
			...
			while ((stack.length > 2) && (precedence <= stack[stack.length - 2].precedence)) {
				right = stack.pop();
				binaryOperator = stack.pop()._operator;
				left = stack.pop();
		...
		expression = stack[i];
		while (i > 1) {
			expression = EBinary(stack[i - 1]._operator, stack[i - 2], expression);
			i -= 2;
		}
	...
	function gobbleToken():Null<Expression> {
		...
		if (Chars.isDecimalDigit(charCode) || charCode == Chars.PERIOD_CODE) {
			...
			// Try to gobble unary _operator expression
	...
	function gobbleVariable():Expression {
		...
		if (index < length) {
			...
			while (index < length
				&& (charCode == Chars.PERIOD_CODE || charCode == Chars.OPEN_BRACKET_CODE || charCode == Chars.OPEN_PAREN_CODE)) {
				index++;
				...
				if (charCode == Chars.PERIOD_CODE) {
					...
					var arguments = gobbleArguments(Chars.CLOSE_PAREN_CODE).map(function(expression) return expression.toExpressionType());
	...
	function gobbleArray():Expression {
		index++;
		var items = gobbleArguments(Chars.CLOSE_BRACKET_CODE).map(function(expression) return expression.toExpressionType());
		return EArray(items);
	}
}
