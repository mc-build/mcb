...
class Register {
	...
	@:keep public static function createStatic<T>(obj:{}, name:String, get:() -> T) {
		var value:T = null;
	...
	@:keep public static function iterator<T>(a:Array<T>):Void->Iterator<T> {
		return if (!untyped Array.isArray(a)) js.Syntax.code('typeof a.iterator === "function" ? a.iterator.bind(a) : a.iterator') else mkIter.bind(a);
	}

	@:keep public static function getIterator<T>(a:Array<T>):Iterator<T> {
		return if (!untyped Array.isArray(a)) js.Syntax.code('a.iterator()') else mkIter(a);
	}
