...
for (const test of tests) {
  ...
  for (const version of versionMatrix) {
    ...
    try {
      const io = new TestIo();
      const compiler = new Compiler("", { internalScoreboardName: "_internal_scoreboard", formatVersion: version });
      compiler.io = io;
      ...
      for (const source of test.sources) {
        ...
        const ast = ext === ".mcbt" ? Parser.parseMcbtFile(tokens) : Parser.parseMcbFile(tokens);
      ...
      if (version === FeatureFlags.latestVersionSentinel) {
        try {
          ...
          for (const [fileName, ast] of transformed.entries()) {
            astSegments.push(`### ${fileName}\n${AstStringifier.stringify(ast)}`);
          }
          ensureDirectory(test.resultPath);
          fs.writeFileSync(path.join(test.resultPath, `${version}.ast.mcb`), astSegments.join("\n"), "utf8");
        } catch (error) {
          ensureDirectory(test.resultPath);
          fs.writeFileSync(path.join(test.resultPath, `${version}.ast.mcb`), String(error), "utf8");
        }
      }

      const expected = test.expectedResult.get(version) ?? null;
      const normalizedActual = normalizeLineEndings(result);
      const normalizedExpected = expected !== null ? normalizeLineEndings(expected) : null;

      if (normalizedExpected === null || WRITE_FLAG) {
        ensureDirectory(test.resultPath);
        fs.writeFileSync(path.join(test.resultPath, `${version}.txt`), result, "utf8");
      ...
      if (normalizedExpected === normalizedActual) {
        ...
        console.error(`  Expected at ${path.join(test.resultPath, `${version}.txt`)}`);
        console.error(`  Actual written to ${actualPath}`);
      }
    } catch (error) {
      hadFailure = true;
      ensureDirectory(test.resultPath);
      fs.writeFileSync(path.join(test.resultPath, `${version}.txt`), String(error), "utf8");
