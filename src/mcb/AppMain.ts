...
function resolveTemplateDirectory(): string {
  ...
  throw new Error("Unable to locate template directory. Please ensure the package was built correctly.");
}
...
function loadConfig(resolvedPath: string): Record<string, unknown> {
  if (!fs.existsSync(resolvedPath)) {
    Logger.warn(`Config file not found, using default config (${resolvedPath}).`);
    return {};
  }
...
async function fetchVersionSummary(): Promise<JsonSummary> {
  return new Promise((resolve, reject) => {
    https
      .get("https://raw.githubusercontent.com/misode/mcmeta/summary/version.json", (res) => {
        if (res.statusCode && res.statusCode >= 400) {
          reject(new Error(`Failed to fetch version summary: ${res.statusCode}`));
          res.resume();
          return;
        }
...
function applyTemplate(content: string, values: Record<string, string | number>): string {
  let result = content;
...
async function executeCallback(callback: DoneCallback | undefined, compiler: Compiler): Promise<void> {
  if (!callback) {
    return;
  }
...
function handleUpdatingFilesBasedOnCache(io: SyncIo, cache: Map<string, string>): Map<string, string> {
  ...
  for (const file of [...removed]) {
    ...
    if (!seenDirs.has(dir) && fs.existsSync(dir) && fs.statSync(dir).isDirectory()) {
      seenDirs.add(dir);
    }
  }
...
async function runCompile(opts: BuildOpts, callback?: DoneCallback): Promise<Compiler> {
...
export async function compile(opts: BuildOpts, callback?: DoneCallback): Promise<Compiler> {
...
export async function generate(outfile: string, arg: { libDir: string; baseDir: string; configPath: string }): Promise<void> {
