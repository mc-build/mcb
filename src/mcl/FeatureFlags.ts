...
export class FeatureFlags {
	...
	static getAvailableVersions(includeLatestSentinel = true): number[] {
		const versions = Array.from(FeatureFlags.flags.keys()).sort(
			(a, b) => a - b,
		);
	...
	apply(version: number, overrides: FeatureFlagOverrides): void {
		...
		for (const id of ids) {
			...
			if (
				flag?.useFolderRenames48 !== undefined &&
				flag.useFolderRenames48 === true
			) {
		...
		if (overrides) {
			if (
				overrides.useFolderRenames48 !== undefined &&
				overrides.useFolderRenames48 === true
			) {
