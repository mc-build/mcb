...
export type TemplateArgumentConstructor = new (name: string | null, pos: PosInfo) => TemplateArgument;

type InlineEvaluator = (script: string, context: CompilerContext, pos: PosInfo) => unknown;

type BlockEvaluator = (code: string, context: CompilerContext, pos: PosInfo, isTLD?: boolean) => void;

type TransformEvaluator = (code: string, context: CompilerContext, pos: PosInfo, isTLD?: boolean) => unknown;
...
export abstract class TemplateArgument {
  ...
  static parse(source: string, pos: PosInfo): TemplateArgument {
    ...
    const type = colonIndex === -1 ? "literal" : source.substring(colonIndex + 1);
  ...
  static evaluateInlineExpression(script: string, context: CompilerContext, pos: PosInfo): unknown {
  ...
  abstract parseValue(value: string, pos: PosInfo, context: CompilerContext): TemplateParseResult;

  parseValueBlock(_value: unknown, _pos: PosInfo, _context: CompilerContext): TemplateParseResult {
