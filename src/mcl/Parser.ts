...
function format(template: string, ...data: Array<string | number | null | undefined>): string {
  let result = template;
...
function unreachable(token: Token): ParserError {
  switch (token.type) {
    case "Literal":
      return new ParserError(format("Unexpected token '{}' at {}:{}:{}", token.v, token.pos.file, token.pos.line, token.pos.col));
    case "BracketOpen":
      return new ParserError(format("Unexpected '{' with data '{}' at {}:{}:{}", token.data ?? "", token.pos.file, token.pos.line, token.pos.col));
    case "BracketClose":
      return new ParserError(format("Unexpected '}' at {}:{}:{}", token.pos.file, token.pos.line, token.pos.col));
  }
}
...
function block(reader: TokenInput, sub: () => void, allowData = true, onLastToken?: (token: Token) => void): string | null {
...
function json(reader: TokenInput): AstNode {
  ...
  return { type: "Raw", pos: start, value: result, continuations: [], isMacro: false };
}
  ...
  while (reader.hasNext()) {
    ...
    if (token.type === "Literal") {
      ...
      if (StringUtils.startsWithConstExpr(value, "template ")) {
        ...
        nodes.push({ type: "Import", pos: token.pos, name: value.substring("import ".length) });
      } else {
        throw unreachable(token);
      }
    } else {
...
function parseTLD(reader: TokenInput): AstNode {
  ...
  if (StringUtils.startsWithConstExpr(value, "dir ") && reader.peek().type === "BracketOpen") {
    const body: AstNode[] = [];
    const data = block(reader, () => body.push(parseTLD(reader)), false);
    if (data !== null) {
      throw unreachable(token);
    }
    return { type: "Directory", pos, name: value.substring("dir ".length), body };
  }
  if (StringUtils.startsWithConstExpr(value, "<%%")) {
    const script: Token[] = [];
    while (true) {
  ...
  return { type: "JsonFile", pos, name, info: { kind: "Tag", subType: type, replace, entries } };
  }
  if ([
    "advancement ",
    "enchantment ",
  ...
  if (StringUtils.startsWithConstExpr(value, "worldgen ")) {
    ...
    const entries: AstNode[] = [];
    block(reader, () => entries.push(innerParse(reader)));
  return { type: "JsonFile", pos, name: payload, info: { kind: "WorldGen", subType: subtype, name: payload, entries } };
  }
  throw unreachable(token);
}

function readPlainJsonFile(v: string, pos: PosInfo, reader: TokenInput): AstNode {
  const bits = v.split(" ").filter((x) => x !== "");
  const type = bits.shift() ?? "";
  const name = bits.shift() ?? "";
  const entries: AstNode[] = [];
  block(reader, () => entries.push(json(reader)));
  return { type: "JsonFile", pos, name, info: createJsonInfo(type, entries, pos) };
}

function createJsonInfo(type: string, entries: AstNode[], pos: PosInfo): JsonTagType {
  switch (type) {
    case "advancement":
    ...
    default:
      throw new ParserError(format("\"Unsupported json file type '{}' at {}:{}:{}", type, pos.file, pos.line, pos.col));
  }
}
...
function innerParse(reader: TokenInput): AstNode {
  ...
  if (token.type === "Literal") {
    ...
    if (StringUtils.startsWithConstExpr(value, "function ")) {
      ...
      const spaceIdx = payload.indexOf(" ");
      const name = payload.substring(0, spaceIdx === -1 ? payload.length : spaceIdx);
    ...
    if (StringUtils.startsWithConstExpr(value, "execute") && (value.charAt("execute".length) === " " || value.charAt("execute".length) === "<")) {
    ...
    if (value === "block" || StringUtils.startsWithConstExpr(value, "block ")) {
      ...
      return { type: "Block", pos, name: name.length > 0 ? name : undefined, body, data: data ?? undefined, isMacro, isInline: false };
    }
    if (StringUtils.startsWithConstExpr(value, "return run")) {
      const sub = value.substring("return run ".length).trim();
      const newPos: PosInfo = { file: pos.file, line: pos.line, col: pos.col + "return run ".length };
      const peek = reader.peek();
      if (peek.type === "BracketOpen" && sub.length === 0) {
        const body: AstNode[] = [];
        const data = block(reader, () => body.push(innerParse(reader)));
        return { type: "ReturnRun", pos, value: { type: "Block", pos, name: undefined, body, data: data ?? undefined, isMacro: false, isInline: false }, isMacro };
      }
      reader.back();
  ...
  if (token.type === "BracketOpen") {
    const body: AstNode[] = [];
    const data = block(reader, () => body.push(innerParse(reader)));
    return { type: "Block", pos: token.pos, name: undefined, body, data: data ?? undefined, isMacro: false, isInline: false };
  }
  throw unreachable(token);
}

function readRaw(pos: PosInfo, v: string, reader: TokenInput, isMacro: boolean): AstNode {
  if (!reader.hasNext()) {
  ...
  while (true) {
    ...
    if (peek.type === "Literal" && peek.pos.line === line) {
      reader.skip();
      continuations.push({ type: "Raw", pos: peek.pos, value: peek.v, continuations: [], isMacro: false });
    } else if (peek.type === "BracketOpen" && peek.pos.line === line) {
      const body: AstNode[] = [];
      const data = block(reader, () => body.push(innerParse(reader)));
      continuations.push({ type: "Block", pos: peek.pos, name: undefined, body, data: data ?? undefined, isMacro: false, isInline: false });
...
function parseCompileTimeIf(v: string, pos: PosInfo, reader: TokenInput, arg: () => AstNode): AstNode {
  ...
  while (reader.hasNext()) {
    const peek = reader.peek();
    if (peek.type === "Literal" && (peek.v === "ELSE" || StringUtils.startsWithConstExpr(peek.v, "ELSE "))) {
      reader.skip();
      let condition = peek.v === "ELSE" ? null : peek.v.substring("ELSE ".length).trim();
...
function parserCompilerLoop(v: string, pos: PosInfo, reader: TokenInput, handler: () => AstNode): AstNode {
  const body: AstNode[] = [];
  ...
  if (match) {
    const loop = match[1];
    const as = match[2];
    const vars = as.length === 0 ? undefined : as.split(",").map((x) => x.trim());
...
function parseExecute(v: string, pos: PosInfo, reader: TokenInput, isMacro: boolean): AstNode {
  if (reader.hasNext() && reader.peek().type === "BracketOpen") {
    const body: AstNode[] = [];
    if (!v.endsWith("run") && executeRegExp.test(v)) {
      const match = executeRegExp.exec(v);
      if (match) {
        const insertPos: PosInfo = { file: pos.file, line: pos.line, col: pos.col + match.index + match[0].length };
        reader.insert({ type: "Literal", v: v.substring(match.index + match[0].length).trimLeft(), pos: insertPos });
        return { type: "Execute", pos, command: v.substring(0, match.index + 3).trimRight(), value: innerParse(reader), isMacro };
      }
    }
    const data = block(reader, () => body.push(innerParse(reader)));
    const continuations: AstNode[] = [];
    while (reader.hasNext()) {
      ...
      if (StringUtils.startsWithConstExpr(text, "else $") && text.endsWith("run")) {
        reader.skip();
        ...
        continuations.push({
          type: "ExecuteBlock",
          pos: { file: blockPos.file, line: blockPos.line, col: blockPos.col + 5 },
          execute: command,
          data: elseData ?? undefined,
          body: elseBody,
...
function parseSchedule(v: string, pos: PosInfo, reader: TokenInput, isMacro: boolean): AstNode {
  ...
  if (StringUtils.startsWithConstExpr(payload, "function ")) {
    ...
    const name = target.substring(0, spaceIdx === -1 ? target.length : spaceIdx);
    ...
    if (!delay) {
      throw new ParserError(format("\"Expected delay after function name in schedule command\" at {}:{}:{}", pos.file, pos.line, pos.col));
    }
    return { type: "ScheduleCall", pos, delay, target: name, mode, isMacro };
  }
  if (StringUtils.startsWithConstExpr(payload, "clear ")) {
    return { type: "ScheduleClear", pos, target: payload.substring("clear ".length), isMacro };
  }
  const spaceIdx = payload.indexOf(" ");
