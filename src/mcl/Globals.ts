type LoopHandler = (...args: unknown[]) => IterableIterator<unknown> | Iterator<unknown> | unknown;
...
class McFloatIterator implements IterableIterator<number> {
  ...
  next(): IteratorResult<number> {
    const done = this.step > 0 ? this.current > this.max : this.current < this.max;
    if (done) {
...
class McIntIterator implements IterableIterator<number> {
  ...
  next(): IteratorResult<number> {
    const done = this.step === 1 ? this.current > this.max : this.current < this.max;
    if (done) {
      return { done: true, value: undefined };
    }
    const value = this.current;
...
const loopVariants: Array<{ signature: string; handler: LoopHandler }> = [
  ...
  },
  {
    signature: "function",
    handler: (...args: unknown[]) => {
      const [factory] = args as [() => IterableIterator<unknown> | Iterator<unknown>];
      const iterator = factory();
      if (typeof (iterator as { [Symbol.iterator]?: () => Iterator<unknown> })[Symbol.iterator] === "function") {
        return iterator;
      }
...
function selectLoopVariant(args: unknown[]): LoopHandler {
  ...
  throw new Error(`Invalid arguments for REPEAT (${args.map((v) => typeof v).join(", ")})`);
}
