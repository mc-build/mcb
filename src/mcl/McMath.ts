...
type FlattenedNode =
  ...
  | { kind: "Binary"; operator: string; left: FlattenedNode; right: FlattenedNode }
  | { kind: "Add"; items: FlattenedNode[] }
  | { kind: "Mul"; items: FlattenedNode[] }
  | { kind: "Call"; callee: string; args: FlattenedNode[] }
  | { kind: "Conditional"; test: FlattenedNode; consequent: FlattenedNode; alternate: FlattenedNode };
...
function tokenizeExpression(expression: string, context: CompilerContext): Token[] {
  const tokens: Token[] = [];
  let index = 0;
  ...
  while (index < expression.length) {
    ...
    if (char >= "0" && char <= "9") {
      ...
      while (cursor < expression.length) {
        ...
        if (digit === ".") {
          throwError("Decimal literals are not supported in eq expressions", cursor);
        }
        break;
      }
    ...
    if (isIdentifierStart(char)) {
      let value = char;
      let cursor = index + 1;
      while (cursor < expression.length && isIdentifierPart(expression.charAt(cursor))) {
...
function parseNormalizedExpression(expression: string, context: CompilerContext): ExpressionNode {
  ...
  const unexpected = (token: Token | null): never => {
    if (!token) {
      throw CompilerError.create("Unexpected end of eq expression", null, context);
    }
    throw CompilerError.create(`Unexpected token '${token.value}' in eq expression`, null, context);
  };
  ...
  const parsePrimary = (): ExpressionNode => {
    ...
    if (token.type === "paren" && token.value === "(") {
      ...
      if (!closing || closing.type !== "paren" || closing.value !== ")") {
        throw CompilerError.create("Unmatched '(' in eq expression", null, context);
      }
...
export function compile(equation: string, context: CompilerContext): McMathCompileResult {
  ...
  if (!lhsTarget) {
    throw CompilerError.create("eq command is missing a scoreboard target", null, context);
  }
  skip();
  const lhsObjective = collect();
  if (!lhsObjective) {
    throw CompilerError.create("eq command is missing a scoreboard objective", null, context);
  }
  skip();
  const separator = collect();
  if (!separator) {
    throw CompilerError.create("eq command is missing an operator", null, context);
  }
  if (!SCOREBOARD_SEPARATORS.has(separator)) {
    throw CompilerError.create(`Unsupported eq operator '${separator}'`, null, context);
  }
  skip();
  const rhs = working;
  if (rhs.length === 0) {
    throw CompilerError.create("eq command is missing a right-hand side expression", null, context);
  }

  const { normalizedEquation, placeholderToScoreboard } = normalizeEquation(rhs, context);
  ...
  try {
    ...
    throw CompilerError.create(`Failed to compile eq expression: ${String(error)}`, null, context);
  }
}
...
function normalizeEquation(rhs: string, context: CompilerContext): {
  ...
  while (idx < rhs.length) {
    ...
    const code = char.charCodeAt(0);
    const isStart = char === "@" || (code >= 97 && code <= 122) || (code >= 65 && code <= 90);
    ...
    if (isStart) {
      ...
      if (idx >= rhs.length || rhs.charAt(idx) !== " ") {
        throw CompilerError.create("Scoreboard entries must contain a space between name and objective", null, context);
      }
      idx++;
      ...
      if (second.length === 0) {
        throw CompilerError.create("Malformed scoreboard reference in eq expression", null, context);
      }
...
function organize(node: ExpressionNode): ExpressionNode {
  switch (node.type) {
    ...
    case "Identifier":
      return { ...node };
    case "Unary":
      return { type: "Unary", operator: node.operator, operand: organize(node.operand) };
    case "Binary": {
      ...
      if (left.type === "Literal" && Number.isInteger(left.value)) {
        ...
        switch (node.operator) {
          ...
          default:
            return { type: "Binary", operator: node.operator, left: organizedLeft, right: organizedRight };
        }
      }

      if (node.operator === "-" && right.type === "Literal" && Number.isInteger(right.value)) {
        return {
          type: "Binary",
          operator: "+",
          left: organizedLeft,
    ...
    case "Call":
      return { type: "Call", callee: node.callee, args: node.args.map(organize) };
    case "Conditional":
      return {
        type: "Conditional",
...
function flatten(node: ExpressionNode): FlattenedNode {
  switch (node.type) {
    ...
    case "Unary":
      return { kind: "Unary", operator: node.operator, operand: flatten(node.operand) };
    case "Binary":
      if (node.operator === "+") {
    ...
    case "Call":
      return { kind: "Call", callee: node.callee, args: node.args.map(flatten) };
    case "Conditional":
      return {
        kind: "Conditional",
...
function render(
  ...
  const isConstant = (node: FlattenedNode): node is { kind: "Literal"; value: number } => node.kind === "Literal";
  const isVariable = (node: FlattenedNode): node is { kind: "Identifier"; name: string } => node.kind === "Identifier";
  ...
  const permanent = (target: string): RenderValue => ({ target, temporary: false });
  const temporaryValue = (target: string): RenderValue => ({ target, temporary: true });
  ...
  const getVariableValue = (node: FlattenedNode): RenderValue => {
    ...
    if (!scoreboard) {
      throw new Error(`Unknown scoreboard placeholder '${node.name}' in eq expression`);
    }
  ...
  const cloneInto = (destination: string, source: RenderValue) => {
    if (source.target.endsWith(` ${constObjective}`)) {
      ...
      commands.push(`scoreboard players operation ${destination} = ${source.target}`);
    }
  };

  const makeTemp = (source: RenderValue, preferredTarget: string | null = null): RenderValue => {
  ...
  const ensureSafe = (value: RenderValue, preferredTarget: string | null): RenderValue => {
  ...
  const renderNode = (node: FlattenedNode, preferredTarget: string | null): RenderValue => {
    switch (node.kind) {
      case "Unary": {
        ...
        if (node.operator === "-") {
          ...
          commands.push(`scoreboard players operation ${mutable.target} *= -1 ${constObjective}`);
        } else if (node.operator !== "+") {
          throw new Error(`Unsupported unary operator '${node.operator}' in eq expression`);
        }
        return mutable;
      }
      case "Binary": {
        ...
        commands.push(`scoreboard players operation ${mutableLeft.target} ${node.operator}= ${rightValue.target}`);
        release(rightValue);
        return mutableLeft;
      }
      case "Add": {
        ...
        for (let index = 1; index < node.items.length; index++) {
          ...
          if (isVariable(entry)) {
            commands.push(`scoreboard players operation ${accumulator.target} += ${getVariableValue(entry).target}`);
          } else if (isConstant(entry)) {
            const value = getValue(entry);
            if (value > 0) {
              commands.push(`scoreboard players add ${accumulator.target} ${value}`);
            } else if (value < 0) {
              commands.push(`scoreboard players remove ${accumulator.target} ${-value}`);
            }
          } else {
            const valueResult = renderNode(entry, null);
            commands.push(`scoreboard players operation ${accumulator.target} += ${valueResult.target}`);
            release(valueResult);
          }
        }
        return accumulator;
      ...
      case "Mul": {
        ...
        for (let index = 1; index < node.items.length; index++) {
          ...
          commands.push(`scoreboard players operation ${accumulator.target} *= ${multiplicand.target}`);
      ...
      case "Call":
        throw CompilerError.create("Function calls are not supported in eq expressions", null, context);
      case "Conditional":
        throw CompilerError.create("Conditional expressions are not supported in eq expressions", null, context);
      default:
  ...
  const applyResult = () => {
    ...
    if (finalOp === "=") {
      ...
      if (variableRoot) {
        commands.push(`scoreboard players operation ${result} = ${getVariableValue(root).target}`);
        return;
      }

      const stored = renderNode(root, result);
      if (stored.target !== result) {
        commands.push(`scoreboard players operation ${result} = ${stored.target}`);
      }
      release(stored);
      return;
    }
    ...
    if (literalRoot) {
      ...
      switch (finalOp) {
        ...
        case "*=":
          ...
          if (value === -1) {
            adjustConstantUsage(-1);
            commands.push(`scoreboard players operation ${result} *= -1 ${constObjective}`);
            return;
          }
          commands.push(`scoreboard players operation ${result} *= ${getConstantValue(root).target}`);
          return;
        case "/=":
          if (value === 0) {
            throw CompilerError.create("Division by zero in eq expression", null, context);
          }
          if (value === 1) {
            return;
          }
        ...
        case "%=":
          if (value === 0) {
            throw CompilerError.create("Modulo by zero in eq expression", null, context);
          }
          if (value === 1) {
            commands.push(`scoreboard players set ${result} 0`);
            return;
          ...
          return;
        default:
          throw new Error(`Unsupported operator '${finalOp}' for literal eq expression`);
      }
    }

    if (variableRoot) {
      commands.push(`scoreboard players operation ${result} ${finalOp} ${getVariableValue(root).target}`);
      return;
    }

    const preferredTarget = finalOp === "=" ? result : null;
    const stored = renderNode(root, preferredTarget);
    commands.push(`scoreboard players operation ${result} ${finalOp} ${stored.target}`);
    release(stored);
  };
